...
```
docker-compose -f labs/docker/networking/compose.yml up -d
...
docker exec networking-web-1 nslookup api-server

# ping the IP address of the API container :
docker exec networking-web-1 ping -c3 <api-server-ip>

# try calling the API inside the web container:
docker exec networking-web-1 wget -qO- api-server:8000/api/customers

# try again with json_pp to see the formatted output:
docker exec networking-web-1 wget -qO- api-server:8000/api/customers | json_pp
```
...
> In case the image is not available you need to build it again following the instructions from the [multi-stage example](../multi-stage/README.md#simple-go-application)

```shell
docker run -d -p 8080:80 whoami:latest
...
```
docker run -d -p 8081:80 whoami:latest
docker run -d -p 8082:80 whoami:latest
docker run -d -p 8083:80 whoami:latest
```
...
```
# on macOS/Linux:
scriptsPath="${PWD}/labs/docker/networking/scripts"
chmod +x "${PWD}/labs/docker/networking/scripts/network-info.sh"
...
```
docker network create --subnet=10.10.0.0/16 dockercourse

docker run --network dockercourse --ip 10.10.0.100 -v ${scriptsPath}:/scripts alpine sh /scripts/network-info.sh
```
...
- [compose-network.yml](./compose-network.yml) - the customers list app using custom IP addresses with a named network
...
```
# update the app:
docker-compose -f labs/docker/networking/compose-network.yml up -d

# try it out at http://localhost:8090

# print the container's network details:
docker exec networking-api-server-1 sh /scripts/network-info.sh
```
...
When you run apps with Compose it builds container names like `networking-api-server-1` - so how do containers discover each other using the Compose service name?

Scale up the API server to run multiple containers and check that the website load-balances requests across them all; then look at the containers' network setup to see how it works.
